...
,	_queries = {
  ...
  getAllUsers : 'SELECT * FROM profiles'
};
...
var ProfileDao = CrudDao.extend({
  ...
  findByUsername: function (username, callbackFn) {
    if (!_.isFunction(callbackFn)) { return; }
    var _this = this;
  ...
  getAllUsers: function (callbackFn) {
    ...
    this.connect(function (err, client) {
      var users = [];
      client
        .query(_this.queries.getAllUsers)
        .on('row', function (row) { users.push(_this.queryToObj(row)); })
        .on('end', function () { callbackFn(users); });
    });
  }
});
